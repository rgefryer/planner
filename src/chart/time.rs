...
impl ChartTime {
    ...
    pub fn new(desc: &str) -> Result<ChartTime, String> {
        ...
        match v[0].parse::<u32>() {
            ...
            Err(e) => {
                return Err(format!("Failed to parse week from time string:{}, {}",
                                   desc,
                                   e.to_string()));
            }
        }
            ...
            match v[1].parse::<u32>() {
                Ok(num) => {
                    if num >= 1 && num <= 5 {
                        ...
                        return Err(format!("Failed to parse day from time string:{}, value is \
                                            out of range",
                                           desc));
                    }
                }
                Err(e) => {
                    return Err(format!("Failed to parse day from time string:{}, {}",
                                       desc,
                                       e.to_string()));
                }
            }
            ...
            match v[2].parse::<u32>() {
                Ok(num) => {
                    if num >= 1 && num <= 4 {
                        ...
                        return Err(format!("Failed to parse quarter from time string:{}, value \
                                            is out of range",
                                           desc));
                    }
                }
                Err(e) => {
                    return Err(format!("Failed to parse quarter from time string:{}, {}",
                                       desc,
                                       e.to_string()));
                }
            }
