...
fn nodes_test() {
	...
	let fred = root_ref.get_node_at_line(1).unwrap().upgrade().unwrap();
	  let child = root_ref.get_node_at_line(5).unwrap().upgrade().unwrap();
	    let grandchild = root_ref.get_node_at_line(7).unwrap().upgrade().unwrap();
	      let greatgrandchild = root_ref.get_node_at_line(8).unwrap().upgrade().unwrap();
	...
	assert_eq!(sibling2grandchild.borrow().get_scheduling_strategy(), SchedulingStrategy::Serial);
	assert_eq!(grandchild.borrow().get_scheduling_strategy(), SchedulingStrategy::Parallel);
	assert_eq!(greatgrandchild.borrow().get_scheduling_strategy(), SchedulingStrategy::Parallel);
	...
	assert_eq!(greatgrandchild.borrow().get_commitments()[0].0.get_quarter(), 30);
	assert_eq!(greatgrandchild.borrow().get_commitments()[0].1.quarters(), 6);
	assert_eq!(greatgrandchild.borrow().get_commitments()[1].0.get_quarter(), 60);
	assert_eq!(greatgrandchild.borrow().get_commitments()[1].1.quarters(), 5);
	...
	let s2byname = root_ref.find_child_with_name("sibling2").unwrap().upgrade().unwrap();
